    ...
    if show_tree and indices is not None:
        ...
        for i, p_idx in enumerate(parents):
            if p_idx in idx_map:
                ...
                arc = get_geodesic_arc(z_2d[i], z_2d[p_i], n_points=15)
                path_data = []
                ...
                svg.append(f'<path d="{path_str}" fill="none" stroke="#4a4e57" stroke-width="0.5" stroke-opacity="0.2"/>')

    # 2. Points (with glowing effect)
    for v in range(10):
        mask = valuations == v
        if not mask.any():
            continue
...
def save_poincare_disk_svg(
    ...
    content = render_poincare_disk_svg(z_2d, valuations, indices, title, colors, show_tree)
